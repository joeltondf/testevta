...
class OmieCategoria
{
    ...
    public function upsert(array $data): void
    {
        ...
        if ($codigo === '' || $descricao === '') {
            throw new InvalidArgumentException('Código e descrição são obrigatórios para a categoria Omie.');
        }
    ...
    {
        $stmt = $this->pdo->query('SELECT ' . self::BASE_COLUMNS . ' FROM omie_categorias ORDER BY descricao');
    ...
    public function getActiveOrdered(): array
    {
        $stmt = $this->pdo->prepare(
            'SELECT ' . self::BASE_COLUMNS . ' FROM omie_categorias WHERE ativo = 1 ORDER BY descricao'
        );
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT ' . self::BASE_COLUMNS . ' FROM omie_categorias WHERE id = :id LIMIT 1'
        );
        $stmt->execute([':id' => $id]);
